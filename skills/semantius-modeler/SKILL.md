...
- ❌ The skill folder (`.claude/skills/semantius-modeler/`). Past sessions have leaked files like `_ats_deploy_entities.sh`, `_seed_v2.ts`, `_apply_rules.sh` into this folder — that's a discipline failure, not a convention. The skill folder is read-only at runtime; only the maintainer edits it.
...
The spec also carries `blueprint_version: "3.0"` (the blueprint artifact version the analyst was working against; the `2.x → 3.0` bump added the §3 `canonical code` and `entity_type` columns, surfaced in the spec as the per-entity `**Catalog entity code:**` / `**Entity type:**` lines). The modeler doesn't re-validate this against the architect; the analyst did.
...
**Modeler v4.0 contract additions (in lockstep with analyst `4.1`).** The spec format gained six new contract carriers; the deployer learns to consume each one and adds five new sub-stages:

1. **New frontmatter keys** — `tagline`, `description`, `persona`, `license`, `module_kind`. Stage 1 parses them; `module_kind` surfaces in the Stage 3 plan-summary line (purely informational, no behavior switch). `tagline` / `description` / `license` pass through to the spec without modeler-side provisioning today — if/when the platform exposes `modules.tagline` / `modules.long_description` / `modules.license` columns, Stage 4a-scaffold step 5 (`update_module`) extends to populate them. `persona` (a flat list) drives Stage 4k persona provisioning as the authoritative create-list; §9.1 RACI carries the per-persona grant detail.
2. **§3 `**Reconciliation:** re-prefixed-from <canonical-module>.<verb>` annotation** (new) — emitted by the analyst on workflow gates / pattern-flag overrides for `embedded_master` entities whose canonical owner module is absent. Stage 2i detects these and queues them for Stage 4n reconciliation when the canonical owner later installs.
3. **§5 `delete_mode` + `fk_format` columns** — already covered by the existing field-shape parse; the deployer consumes verbatim, no derivation. **Stage 4d extension:** skip FK column emission when a §5 edge has `delete_mode: none (required-if-present)` AND the target entity isn't installed in the live catalog AND isn't in this deploy. Today's behavior would create a broken non-null restrict; the new behavior emits no column and no constraint.
4. **§6 `transition` column** (new) — Stage 4m (NEW) reads `<to_state> _(<event_category>)_` and wires the platform's lifecycle-trigger registry (when supported); otherwise emits a 🟡 informational row.
5. **§7 ⚠ annotations** (new) — `⚠ state-machine shape: <reason>` and `⚠ unresolved gate: <reason>`. The deployer SKIPS or FAILS LOUD on annotated rows; never silently provisions a malformed state machine or a phantom gate.
6. **§9 governance section** (new) — Stage 4k (persona provisioning), Stage 4l (functional-ownership default grants), and the existing Stage 2a-scaffold consume §9.

**Permission-prefix resolution rule (v4.0+):** workflow gates and pattern-flag overrides for entity E are prefixed by E's CURRENT owning module slug, not by the installing unit. This rule fires on every install regardless of `module_kind` (the label is informational; the behavior is bundle-agnostic). Stage 4a-scaffold honors it when minting gates / overrides for entities with re-prefixed-from annotations. Stage 4n (NEW) handles the master-install reconciliation: when Branch-B promotion (4c-promote) moves an entity to a new owning module, 4n sweeps every non-canonical-prefixed permission for that entity's verbs, mints sibling canonical-prefixed permissions, and creates sibling `role_permissions` rows for every grant on every non-canonical prefix. No deletes (per the no-auto-deletion symmetric rule). When Plan 5 (entity-scoped identity, `<entity>:<verb>`) lands, 4n becomes a no-op merge.

See the [CHANGELOG.md v4.0 entry](./CHANGELOG.md) for the full deployer-side delta and the Plan-5 escape hatch.

**Modeler v4.1 contract additions (in lockstep with analyst `4.2`).** (At the time, `EXPECTED_MAJOR` stayed `4` — analyst `4.2` was the same major, and the live-RACI engine is an additive, opt-in MINOR with `documentation` mode the default, reproducing v4.0 behavior exactly. `EXPECTED_MAJOR` has since bumped to `5` in lockstep with analyst `5.0` — see the v5.0 CHANGELOG entry.) The spec gained the §9.1 RACI `process_key` + `consult_mode` columns, the §9 `**RACI mode:** living | documentation` line plus a `Processes` catalog, and (living mode only) a `RACI plan` block (`raci_assignments` / `process_gates` / `enforcement_rules`). Stage 1 parses all of these (see the §9 governance sub-indices); Stage 4k consumes them: `documentation` mode compiles the matrix into `role_permissions` exactly as v4.0, while `living` mode additionally materializes the platform's live-RACI tables (`processes` / `raci_assignments` / `process_gates`) and enforcement rules (the `is_raci_actor` A-gate, the `has_consultation` C-block) via `postgrestRequest`, and sets `modules.settings.raci_mode = "living"`. Agent actors (`kind = skill`) resolve to a role held by an `is_agent` user. See the [CHANGELOG.md v4.1 entry](./CHANGELOG.md). **Version-pairing (the axes do not share numbers):** the live-RACI engine is modeler `v4.1` ↔ architect/analyst `4.2` (modeler `v4.0` ↔ architect/analyst `4.1`).

**Provenance stamping (in lockstep with architect `5.0` / analyst `5.1`; `EXPECTED_MAJOR` stays `5`).** Base schema v0.1.2 added core provenance columns on `entities` / `modules` / `roles`, and the spec now carries per-entity `**Catalog entity code:**` / `**Entity type:**` / `**Catalog alias:**` / `**Canonical owner:**` lines (analyst v5.1). The deployer is the only catalog writer, so it **stamps these values at provision time** — `catalog_entity_code` (canonical, write-once), `canonical_owner_module`, `entity_type` (`'unclassified'` when absent), `pattern_flags` on `create_entity`; **APPENDS** `catalog_entity_aliases` on reuse/merge; `catalog_module_code` + `settings` keys on `create_module` / `update_module`; `catalog_role_code` on `create_role`. It stamps **values only** — never `create_field` these core columns, never write `ctype`. This is additive and MINOR: an un-updated deployer reading a v5.1 spec simply leaves the columns at their empty defaults, and nothing regresses, so `EXPECTED_MAJOR` stays `5`. See "Provenance stamping" in Stage 4 for the full rules.

**Lifecycle state field enforcement + `catalog_field_code` retired (in lockstep with architect `5.2` / analyst `5.3`; `EXPECTED_MAJOR` stays `5`).** Two coordinated changes, neither a spec-format break. (1) **The lifecycle state field is fixed to `workflow_state`.** Stage 1 parse now FAILS LOUD (🛑 High blocker) on any spec whose lifecycle state lands in a field not named `workflow_state` — detected via a `process_gates[].state_column` that isn't `workflow_state`, or a lifecycle-bearing entity (one carrying a `workflow-gate (lifecycle)` permission or a `process_gates` row) whose §3 Fields table has no `workflow_state` enum field. Stage 5 re-verifies the live field name. The deployer never silently renames the field; it routes the user back to the analyst. (2) **`catalog_field_code` is no longer stamped** (the platform is dropping the `fields.catalog_field_code` column); `create_field` drops the value and the remaining write-once codes (`catalog_entity_code` / `catalog_module_code` / `catalog_role_code`) are unchanged. `EXPECTED_MAJOR` stays `5`: a spec that already stores state in `workflow_state` passes the new gate unchanged; a legacy spec with a differently-named state field now fails loudly, which is the intent. See the [CHANGELOG.md](./CHANGELOG.md) entry.
...
```
1. Parse spec  →  2. Inspect Semantius  →  3. Plan & Present  →  4. Execute  →  5. Verify  →  6. Sample Data?
```
...
- **`module_kind`** from YAML frontmatter (v4.0+), optional, informational only. Common values: `domain`, `master`, `starter`. NOT a behavior switch — the deployer's logic is `module_kind`-agnostic. Surface in the Stage 3 plan-summary line `🏷 module_kind = <kind>`; no other consumption. Any unknown value is fine (warned but not blocked).
- **`tagline`** from YAML frontmatter (v4.0+), optional. Carried verbatim; not provisioned to the module record today (no `modules.tagline` column). Follow-up: extend Stage 4a-scaffold step 5 (`update_module`) when the platform exposes the column.
- **`description`** from YAML frontmatter (v4.0+), optional. Longer marketing-voice prose; same posture as `tagline`. The existing `system_description` (≤40-char) is still what populates `modules.description` (see the Module schema note in 2a).
- **`license`** from YAML frontmatter (v4.0+), optional. Catalog metadata; carry only, no provisioning today.
- **`persona`** from YAML frontmatter (v4.0+), flat list. Authoritative create-list for Stage 4k persona provisioning. Each entry is UPPER-CASE hyphen-separated (`RECRUITING-RECRUITER`, `HIRING-MANAGER`); Stage 4k converts to snake_case for the role slug. Cross-check: every name in this list MUST appear in §9.1 RACI `actor` column at least once (Stage 1 enforces; mismatch is a 🛑 blocker).
- **`promotion_decisions`** from YAML frontmatter (analyst v3.0+), optional. A **list** of objects, one per promoted entity: `{entity: <table_name>, host_module: <master_slug>, manage_option: 1|2|3|4}`. Key into it by `entity`: the `promotion_decisions[<entity>]` notation used in Stages 2d / 4c-promote means *the list entry whose `entity` equals `<entity>`*, NOT a map lookup. `manage_option` drives the cross-module manage-inclusion edges (see analyst Stage 3b for the 1–4 semantics); `host_module` names the master to create or join. A mis-read here silently produces the wrong master-inclusion hierarchy with no surface error, so resolve the entry by exact `entity` match and treat a missing entry for a `promote-to-master` entity as a 🛑 blocker (route back to the analyst).
...
- **Permissions catalog** (`## 8.1 Permissions catalog`, mandatory in analyst v4.1+) — parse the table verbatim. Five columns in this fixed order: `permission | tier | description | included in :admin? | reconciliation`. Build a `permissions` index from the rows; each entry `{permission, tier, description, included_in_admin, reconciliation}`. The deployer's Stage 2a (module + permissions setup) creates one `create_permission` call per row in table order, using the `permission` and `description` cells. The rollup chain comes from the **§9.1 Permission hierarchy** table (`permission | includes | reconciliation`): after all permissions exist, the deployer issues one `create_permission_hierarchy` call per §9.1 row, with `including_permission = permission` (the broader, the one doing the including) and `included_permission = includes` (the narrower, the one being included). **Parse-time validation** (all 🛑 High blockers, reject before any write):
  ...
  - every `tier` value in `{baseline-read, baseline-manage, baseline-admin, workflow-gate (lifecycle), workflow-gate (rule), override (personal_content), override (submit_lock), narrow}` (the `narrow` tier was added in analyst v2.1; files stamped older than `2.1` that include it should not exist in the wild, refuse and route to the analyst);
...
- **§9 governance section** (v4.0+, extended v4.2). Parse into these sub-indices:
  ...
  - `raci[]` from §9.1's `RACI realization` table: `{actor, kind: persona|skill, raci: R|A|C|I, process_key, consult_mode, realization, grant_module: <entity's current owning module slug>}`. `process_key` / `consult_mode` are v4.2 additions; the process display name and description are NOT on this row (they live in the `Processes` catalog below, joined by `process_key`). The `grant_module` column (analyst v4.1+ emission) is the entity-owning-module resolution result. Stage 4k uses it directly when resolving each grant's permission code; the deployer does NOT re-resolve.
  - **RACI mode** (`living | documentation`, v4.2) from the §9 `**RACI mode:**` header line (and frontmatter). Drives Stage 4k's branch; absent → Stage 4k applies the catalog-aware fallback default.
  - **Processes catalog** (v4.2) from the §9 `Processes` table: `processes[] {process_key, name, description, ordering}`. The display name + description live here; the RACI rows reference `process_key` only.
  - **RACI plan** (v4.2, living mode only) from the §9 `RACI plan` block: `raci_assignments[] {process_key, role_slug, raci, consult_mode}`, `process_gates[] {process_key, entity, gate_kind, to_state, state_column, emits_events}`, `enforcement_rules[] {entity, rule, jsonlogic}`. Stage 4k materializes these (plus the Processes catalog) via `postgrestRequest`. Each `process_gates[].state_column` MUST be `workflow_state` (the fixed lifecycle state field); reject any other value as a 🛑 per the lifecycle-state-field cross-check above.
...
| `**Catalog entity code:** `<code>`` (v5.1+) | `catalog_entity_code` — the **canonical** code (NOT `table_name`). Stamped on `create_entity` (✨ new / rename-incoming / promote). Write-once: never re-sent on a later `update_entity` rename. Default to `table_name` when the line is absent. |
| `**Entity type:** <class>` (v5.1+) | `entity_type` — the closed 6-value class. Stamped on `create_entity`. `'unclassified'` (never `''`) when absent. |
| `**Label parent:** `<fk_field_name>`` (v5.2+) | `label_parent` — the FK field name that is this entity's identity spine. Passed on `create_entity` / `update_entity` **only when the line is present**; omit the key entirely (leave `null`) when absent. A normal nullable property, NOT a write-once provenance code — re-pointing it is allowed and changes the composed `_label` with no data migration. The deployer does not derive it (the analyst already did); it never targets a junction and never appears on a junction entity. |
| `**Canonical owner:** `<module_slug>`` (v5.1+) | `canonical_owner_module` — the owner-module slug from the spec's `**Canonical owner:**` line, for an `embedded_master` provisioned locally as a placeholder while its canonical owner is absent; `''` when this module owns the entity, it is local, or the line is absent. Stamped on `create_entity`. |
...
| `**Catalog alias:**` line(s) (v5.1+) | `catalog_entity_aliases` — on a reuse/merge onto an existing host, **APPEND** each `{alias_code, source_domain, source_module, decided}` element to the host's array (read-modify-write; never rewrite prior elements). Not set on a plain `create_entity`. |
...
- **(v5.2+)** the composed-label columns: the entity's own `_label` (`ctype: _label`) and every `<fk>_label` companion (`ctype: fk_label`). They are platform-owned, read-time projections — never `create_field` them, and skip them on every field diff. They never appear in the spec's Fields tables or in `read_field`.

> **Title correction (validate, don't blanket-fix):** the platform now derives a sensible `title` for the auto-created `label_column` field, so the old always-run `update_field` pass is gone. After `create_entity`, read the field's live `title` and compare it to the §3 Label for that row; issue `update_field` ONLY for the outliers where they differ. When you do, the `id` is the composite string `"<table_name>.<field_name>"` (pass it as a string, not an integer). Most entities need no correction.
...
4. **Create default roles per tier**, reading each role's `slug` **verbatim from the spec's §9.1 baseline-roles table** (the `role` column is the resolved, deploy-ready slug; the analyst already normalized it to the platform's `roles.slug` rule, so the deployer never reconstructs `<slug>_<tier>` — a module slug may legally carry a hyphen the role slug cannot). Idempotent: `read_role --single` by that §9.1 slug; `create_role` if exit 1, passing the §9.1 `slug` verbatim plus `role_name`, `description`, `module_id`, `origin = "model"`. Then attach the row's `baseline grant` permission to each role with the same read-before-write guard: `read_role_permission --single` by `role_id=eq.<role_id>&permission_id=eq.<perm_id>` → `create_role_permission` only on exit 1.
5. **Populate the six module-record references.** After permissions and roles exist, `update_module` to set `view_permission = "<slug>:read"`, `manage_permission_id`, `admin_permission_id` (nullable), `default_*_role_id`. Write only the fields whose live value differs.
...
### 2h. `module_kind` recognition (v4.0+)
...
### 2i. Canonical-owner detection for re-prefixed permissions (v4.0+)
...
### 2j. `has_single_approver` cross-check (v4.0+)
...
| `🏷 module_kind = <kind>` (v4.0+) | Informational label from frontmatter (`domain` / `master` / `starter` / etc.). No behavior switch. |
| `👥 Personas to provision: <list>` (v4.0+) | Per-persona summary from §9 RACI + frontmatter `persona`. Stage 4k creates each named persona as a tenant role and grants its RACI permissions. Each line names the persona slug and the count of grants. |
| `🏢 Functional ownership: owner = <function>.<role>, contributor = <function>.<role>` (v4.0+) | Summary from §9.2 functional ownership table. Stage 4l grants the named default-tier permission to the named default role for each row. |
| `🔁 Re-prefix: <slug>:<verb> (canonical <module> not installed)` (v4.0+) | The permission row in §8.1 carries a `re-prefixed-from <canonical-module>.<verb>` annotation AND the canonical module isn't installed. Stage 4a-scaffold mints the permission under the installing-unit slug; Stage 2i has queued it for future reconciliation. |
| `🔁 Master-install reconciliation: rename <slug>:<verb> → <canonical-module>:<verb>; migrate <N> grants` (v4.0+) | The canonical module IS installed AND a Branch-B promotion will move the entity. Stage 4n will mint the canonical-prefixed sibling permission, create sibling `role_permissions` rows for every grant on every accumulated non-canonical prefix, and re-emit matching `permission_hierarchy` edges. No deletes. |
| `⚠️ Skipping <entity>.<state> — ⚠ <annotation kind>` (v4.0+) | Per §7 lifecycle row carrying a `⚠ state-machine shape` or `⚠ unresolved gate` annotation. The deployer SKIPS the row (does NOT silently provision). The line is informational + halt-on-execute. |
| `💤 Skipping FK <from>.<target_id> → <target_entity> — target not installed (presence-conditional)` (v4.0+) | Per §5 edge with `delete_mode: none (required-if-present)` AND target absent from live catalog. Stage 4d skips the FK column emission entirely (no broken non-null restrict). |
...
A `select_rule` create / modify or an `edit_permission` tier flip still pauses for explicit confirmation (medium-risk: read-visibility or write-tier change). These are the only mid-flow prompts.
...
1. **Idempotent role check.** Compute the persona slug as lowercased snake-case (UPPER-CASE hyphen-separated → lowercase underscores): `RECRUITING-RECRUITER` → `recruiting_recruiter`. Then `read_role --single by slug=<persona_slug>`.
   ...
   - **Exit 1 (missing)**: `create_role` with `slug = <persona_slug>`, `role_name = <UPPER-CASE original>`, `origin = "model"`, `module_id = <installing-unit module id>` (so the persona role is tracked under whoever first introduces it), and **`catalog_role_code = <UPPER-CASE original persona name>`** (v5.1+ provenance lineage; the catalog persona this role was provisioned from — stamp as a VALUE, never `create_field` the column; write-once).
2. **Grant resolution per RACI row.** For each parsed §9.1 RACI row whose `actor` is this persona AND `kind = persona`:
   - For `raci = responsible | accountable`: walk the row's parsed grant list (from the `realization` column's `grant gates [<list>]` form). For each gate code in the list, **resolve the actual permission code using the row's `grant_module` column** (analyst v4.1+ emitted; the entity-owning-module lookup result). The resolved code = `<grant_module>:<verb>`. NEVER assume the installing-unit prefix — the gate's prefix follows the entity, which may live in another module if a previous install created it there.
   ...
   - For `raci = consulted`: grant an advisory read on the gate's owning module (`<entity_owning_module>:read`). Idempotent. The row's `consult_mode` (`read` / `notify` / `block`) is **not** behaviorally distinguished in `documentation` mode — all three collapse to the advisory-read grant, reproducing v4.0 exactly. The `notify` / `block` semantics are realized only in `living` mode (carried on the `raci_assignments` row and enforced by the `has_consultation` C-block rule + the `emits_events` notify trigger; see the living-mode materialization below).
   - For `raci = informed`: defer to Stage 4m (handoff wiring) — no role grant; the notification is a side effect.
3. **Skill actor rows (v4.1 — no longer deferred).** RACI rows where `kind = skill` resolve to a **role held by an agent user** (`users.is_agent = true`), the agent-native parallel to a persona. Idempotent: `read_role --single by slug=<skill_slug>` → `create_role` on miss (`origin = "model"`); ensure an agent service user exists (`read_user` by a stable external_id → `create_user` with `is_agent = true` on miss) and hold the role via `create_user_role` (idempotent). Then treat the skill's role_id exactly like a persona's for grants and (in living mode) `raci_assignments`. _(The platform's `users.is_agent` shipped with `0210_raci.sql`; the matrix-level "Accountable must be human" guard and JIT agent tokens remain platform Phase 3 — not enforced here.)_
...
In `documentation` mode, none of the above runs — point 2's grant compilation is the whole realization, exactly as v4.0.

**4l. Functional-ownership default grants (v4.0+).** Walk the parsed §9.2 `functional_ownership` index. For each row `{responsibility, business_function, default_role, default_tier}`:
...
**4m. Boundary-crossing handoff wiring (v4.0+).** For each parsed §6.2 outbound and §6.3 inbound handoff row whose `event_category` is `lifecycle` or `state_change`:
...
**4n. Permission reconciliation on owner-module change (v4.0+).** Fires whenever Branch-B promotion (4c-promote) moves an entity to a new owning module AND Stage 2i queued the entity for reconciliation. Per the entity-owning-module rule, the entity's gates / overrides must now bear the new owner's prefix.
...
**Plan-5 escape hatch.** When the platform later moves to entity-scoped identity (`<entity>:<verb>`), 4n becomes a no-op merge — the prefix never changes because there isn't one. The deployer-side reconciliation obligation disappears entirely; this stage becomes documentation of the rule the analyst already enforces at spec-write time.
...
7. **Live RACI engine (living mode only, v4.1+).** When Stage 4k ran in `living` mode, round-trip every artifact it materialized — this is the verify counterpart to the five-batch 4k living-mode write, and without it a mid-sequence 4k failure is invisible to Stage 5:
...
**v4.0+ additional report sections** (rendered immediately after Counters, before the final ✓ line):
...
Re-prefixed permissions (Stage 4a-scaffold, v4.0+):  N permissions
...
Master-install reconciliation (Stage 4n, v4.0+):  P entities reconciled, Q permissions renamed, R grants re-pointed
...
Boundary-crossing handoffs (Stage 4m, v4.0+):  S wired, T unwired (no trigger registry)
...
Skipped FKs (presence-conditional, v4.0+):  U skips
...
Skipped on ⚠ flag (v4.0+):  V skips
  ⚠️ <entity>.<state> — <verbatim reason>

Live RACI engine (Stage 4k living mode, v4.1+):  W processes, X assignments, Y gates, Z enforcement rules
  raci_mode = living          ✓
...
const spring = await post("/campaigns", { campaign_name: "Spring Launch", workflow_state: "active" });
const fall = await post("/campaigns", { campaign_name: "Fall Promo", workflow_state: "draft" });
...
| Model's §8.1 Permissions catalog is missing entirely (file is v1.x or an older analyst skipped the section) | 🛑 High | Reject at Stage 1. v2.0 files require the section; v1.x files are a major-mismatch. Route the user back to analyst Mode D Rebuild to materialize the v2 file from the existing content. Do not attempt to synthesize the table from per-entity `Edit permission:` annotations or any other section, the v2 contract requires the analyst to produce the table as a deliberate authoring step, not a deployer-side inference. |
