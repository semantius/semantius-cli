...
| `label_parent` | §3 `**Label parent:**` — **omit the key entirely** when the line is absent (normal nullable, NOT write-once provenance) | _(no line → omit)_ | omit (live `''`) |
...
await call("create_entity", {
  data: {
    ...
    // label_parent: "<fk_field>",              // §3 Label parent — include ONLY when the line is present
  },
});
```
