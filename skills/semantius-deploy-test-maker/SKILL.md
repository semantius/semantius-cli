...
### Semantius built-in entities (lookup adjustment only)

Semantius ships with a small set of built-in entities the deployer dedups against rather than recreating. The current canonical set is `users` (others may follow). Because the dedup'd row lives in the platform's identity module rather than the current model's module, the verifier's **lookup** for these entities must be unscoped by `module_id`. The script emits a `BUILT_IN_TABLES` constant (currently `["users"]`) at the top of the module so a future major bump can extend it in one place.

**That is the only adjustment.** After the lookup, the verifier checks every property and every field exactly as it does for any other entity. The skill's load-bearing principle ("over-check rather than under-check; any undetected difference is a business risk") forbids any rule that quietly skips checks on a class of entities because the cause of failure is "known".

The model declares the contract. The verifier reports differences. The user — not the verifier, and not the skill author — decides whether each failure is resolved by re-deploying, updating the model, fixing the analyst convention, or accepting documented drift. A failure with a known root cause is still a failure. Concretely:

- If the model declares `users.user_name` and the built-in has `display_name` only, the test reports `users.user_name` missing. That is correct.
- If the model declares `users.email.unique_value=true` and the built-in's `email` column has no unique constraint, the test reports the mismatch. That is correct.
- If the built-in's `module_id` doesn't match the current module's `id`, the test reports the mismatch. That is correct.

Each surfaces a real model/platform disagreement that needs a human-resolved decision. Hiding any of them defeats the purpose of the test.
...
2. **Permissions check.** For every row in the §2 Permissions summary table, plus the implicit baseline rows, confirm a live `permissions` record exists with the exact `permission_name`, `description`, and `module_id`.
3. **Permission-hierarchy check.** For every `Hierarchy parent` value in the table (and for the implicit baseline edges), confirm a live `permission_hierarchy` row exists with the matching `(parent_permission_id, child_permission_id)`.
4. **Roles check.** Confirm the three default roles exist with the right `slug` (the live natural key — **not** `name`, which is the deployer-chosen display label) and `module_id`. Role display name and description are deployer-chosen and not asserted.
...
- **`computed_fields` / `validation_rules` / `select_rule` / `input_type_rule`**: deep value-equality after JSON canonicalization (sort object keys, leave arrays in declared order). The deployer round-trips these byte-for-byte; the script must accept that contract and fail loudly on any divergence.
- **Permission, role, and hierarchy ids**: resolve by natural key (`permissions.permission_name`, `roles.slug`, the `(parent_permission_id, child_permission_id)` pair), not by numeric `id`. Ids drift across instances; natural keys are the model's contract.
