...
Example §3 row (explicit default documents intent even when it matches the first listed value):

```
| status | enum | yes | Status |  | enum_values: `draft`, `active`, `discontinued`; default: "draft" |
```
...
**Mechanics.** Description is a dedicated cell in the §3 row (the 5th column, between Label and Reference / Notes), not an annotation. One short sentence, under ~80 chars, no `key: "value"` syntax. The deployer reads the cell directly and passes it verbatim to `create_field`'s `description` parameter. The Reference / Notes column is for **structured annotations only** (`relationship_label: "<verb>"`; `enum_values:` followed by each value as an inline-code token (required on every `format: enum` field; written as `` enum_values: `a`, `b`, `c` `` — the platform field name `enum_values` is fixed and the values render as inline code so they read as data, not prose; the legacy JSON form `enum_values: ["a", "b", "c"]` is also accepted by the deployer and the audit but the backticked form is canonical and matches what the analyst naturally writes); `default: "<value>"` (always a quoted string); `precision: <n>`; `cube_type: <mode>`; `parent label: "<sing>" / "<plur>"`; `label_column`; `unique`; plus the FK arrow `→ <table> (<card>)`); free prose **never** belongs in Notes now that Description has its own column.
...
- **No backticks** around any identifier or value in the Description cell, on the entity-level Description, or on labels. Two narrow exceptions: (1) **enum values** inside the §3 field-row Description cell, which stay snake_case in inline `code` style to mark them as the field's data rather than metadata (canonical example below); (2) enum values inside the §3 field-row **Reference / Notes cell** as part of the canonical `` enum_values: `a`, `b`, `c` `` annotation.
...
- 🟡 **Identifier leakage in any user-facing prose surface (writing convention #6, analyst v3.4+).** Walk every prose surface listed in writing convention #5 (§1 Overview; §2 entity `Purpose` and Permissions `Description`; **every §3 entity-level `Description` sub-block**; every §3 field-row `Description` cell; every `description` key inside `Computed fields` / `Validation rules` / `Input type rules` / `Select rule` sub-blocks; §6 prose; §7 question bodies; `system_description`; every `singular_label` / `plural_label`; every field `Label`). For each surface, flag:
  - **Any backticks** around an identifier or value. Two narrow exceptions: (1) enum values quoted in inline `code` style **inside a §3 field-row Description cell** (canonical *"Match Status reaches `auto_matched`"* pattern); (2) enum values inside a §3 field-row **Reference / Notes cell** as part of the canonical `` enum_values: `a`, `b`, `c` `` annotation. Backticks anywhere else are a Warning.
