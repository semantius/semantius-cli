...
## Skill version: `CURRENT_VERSION = "2.2"`

This skill stamps every model file it writes with `version: "<CURRENT_VERSION>"` in the front-matter, as a quoted string `"MAJOR.MINOR"` (currently `"2.2"`). The version is the analyst skill's own version at the time of the write, not a property of the model's content. It is the single source of truth for compatibility downstream.
...
**No snake_case identifiers when referring to a sibling field.** The Description column is **user-facing prose** — it renders as helper text under the field in the auto-generated form and surfaces verbatim in `read_field` output that agents read cold. Same rule as §1 Overview: when the description refers to another field on the same entity, name it by its **Label** (or in plain English), not by its `field_name`. The literal column identifier breaks the abstraction (form users see a programming token where they should see English) and looks like a leak from a tool view that wasn't cleaned up for human consumption.
...
| I5 | **Permission-driven mode** (depends on who's viewing) | Field whose UI mode should depend on the *viewer*, not the record state. Signal: §3 prose says "editable only by HR", "compensation hidden from non-managers", "social_security_number masked unless privileged role". Implementation depends on whether the platform's client supplies a permission-check operator inside `input_type_rule` context — if not, this family routes to either a `validation_rules` server-side gate (the field is still server-editable for non-holders but the form should not render it) plus a static `hidden` baseline, or to a separate restricted-edit pattern. **Default behavior: surface this as a §7.1 / §7.2 question rather than fabricating a JsonLogic that may not run** — the platform's client-side permission story for `input_type_rule` is the analyst-skill v2.2 known unknown. | (defer to question or pair with server-side rule) |
...
- **I5 fires** when §3 prose explicitly names a role-restricted view ("HR can edit", "non-managers can't see"). The default action is to flag a §7.1 question, not to emit a rule, until the platform's permission-check operator inside `input_type_rule` is verified.
...
#### 🛑 What `select_rule` cannot do, and what NOT to promise

Before walking the families, understand the hard limit on what the rule can express. **The platform evaluates the JsonLogic body per row, with `$today` / `$now` / `$user_id` as the only reserved variables. There is no documented platform mechanism by which holding a specific permission causes the rule to be skipped for that caller.** Specifically:

- **There is no `has_permission` / `require_permission` operator confirmed for the SELECT context.** `require_permission` is documented inside *`validation_rules`* (the write side), where its behavior is "returns true, or throws". A throw on every write the caller doesn't have permission for is sensible; a throw per row during a SELECT scan would be a disaster, so even if the operator is syntactically available the semantics for read are not the same. Until the platform documents an operator that the SELECT context can use to broaden scope per caller, **the rule body cannot reference permissions**.
- **A `<slug>:view_all_<plural>` permission grant alone does NOT bypass `select_rule`.** Granting a permission only adds it to the holder's role; it does not cause the platform to skip the RLS policy. Any prose that promises "callers holding `<slug>:view_all_X` bypass the filter" is **fabricated unless the user has confirmed with platform docs / engineering that a bypass mechanism exists**. Refuse to generate such prose absent that confirmation; surface a §7.1 🔴 blocker instead.
- **`select_rule` applies uniformly to every caller with `view_permission`.** A rule that returns truthy for "submitter or assignee or unassigned" filters managers identically to regular users. If the design requires managers to see *more* rows than the rule admits, the **rule itself must encode that broadening** (e.g., by referencing a column the manager can satisfy that the regular user cannot) — and that requires a column on the row, not a permission held by the caller.

**The consequence.** Tiered visibility ("regular sees own; manager sees all") cannot be encoded inside a single `select_rule` against the platform as currently documented. The analyst must either (a) accept that `select_rule` applies uniformly and tighten the rule to a shape every caller is willing to live under, or (b) route the broader audience through a different mechanism the user explicitly confirms (a separate Postgres role that runs with `BYPASSRLS`, a separate cube view, a separate entity surface for admins, or `view_permission` granularity that exposes the full table to a different role). When the user has not confirmed which mechanism to use, **the rule degrades to the uniform shape and a §7.1 blocker captures the open design question**. Never generate prose that names a bypass that JsonLogic does not encode.

#### Signal families (five)

| # | Family | Shape test in §3 / Stage 9 / Stage 10 | `select_rule` proposal |
|---|---|---|---|
| S1 | **Personal-content shape** | Co-fires with Stage 8 family 13 / Stage 10 W4. Entity records one user's personal contribution (notes, journal entries, private feedback, individual scorecards). The framing is "the author's own" — not "the team's work product". | Per-row: row matches when caller equals the owner field. **Uniform across all callers with `view_permission`** — no in-JsonLogic elevated-tier escape. If the design requires a broader audience for some roles, surface as a §7.1 blocker (see "What `select_rule` cannot do" above) and propose one of the documented alternatives. |
| S2 | **Tiered audience (the ticket / case-management shape)** | Entity carries both a `submitter_user_id` / `requested_by` / `requester_id` AND an `assignee_user_id` / `assigned_to` / `agent_id`. §3 prose describes a "self-service submits, operations team works" division. | Per-row: row matches when caller is submitter OR caller is assignee OR row is unassigned. **All callers with `view_permission` see the same filter** — agents see the same rows as regular users plus the rows where they are the assignee or where assignee is null. If the design requires managers to see *every* row including those that don't satisfy the predicate, this is a 🛑 architectural decision (see "What `select_rule` cannot do" above): split into a Postgres-role bypass, a separate admin entity, or accept the uniform filter. Do **not** silently promise an "RLS bypass via `view_all` permission" in prose. |
| S3 | **Confidential / restricted flag** | Entity carries a `is_confidential` boolean OR a `visibility` enum (`public`/`restricted`/`internal`) OR a `classification` enum. §3 prose says "restricted records are hidden from default view" / "only owners + admins see confidential rows". | Per-row: visible when `is_confidential=false` OR caller matches the row's owner/assignee. The "admins see confidential rows" branch is the same architectural decision as S1/S2 — encode it in the column (e.g., the row's `visibility` enum includes a value that everyone sees) OR route admins through a different mechanism. The rule body itself stays a pure row predicate. |
| S4 | **Sensitive HR / financial data** | Entity covers compensation, performance scores, disciplinary records, contractor rates, vendor pricing. §3 prose says "only direct manager and HR can read" / "compensation is need-to-know". | Per-row: visible when caller is the record's subject (e.g. `subject_user_id == $user_id`) OR the row's `manager_user_id == $user_id` (when the entity carries a manager FK that resolves to the caller). The "HR can read" branch is the architectural decision again — there is no in-JsonLogic permission check; route HR access through `view_permission` granularity or a Postgres bypass role. |
...
|---|---|---|---|---|---|---|
| `interview_feedback` | W4 fired → `interviewer_user_id` owner | none — read-shared by the panel per §3 | none | none | none | per-row: `interviewer_user_id == $user_id`. **Applies to every caller with `view_permission`.** If HR / admin need to read all feedback, surface as §7.1 architectural decision (see "What `select_rule` cannot do"). |
| `tickets` | none | submitter + assignee FKs present, §3 names self-service / agent tiering | none | none | none | per-row: submitter OR assignee OR unassigned. **Same filter for every caller with `view_permission`.** Manager-sees-all is §7.1. |
| `journal_entries` | W4 fired → `author_id` owner | none | none | none | none | per-row: `author_id == $user_id`. Same uniform-filter constraint; controller / auditor access is §7.1. |
...
#### Architectural decision required when a fired family hits the uniform-filter wall

For every `S*` cell where the design wants tiered visibility but `select_rule` can only express the per-row predicate, **stop and surface a §7.1 🔴 blocker** before continuing. The blocker names:

1. The entity and the family that fired.
2. The per-row predicate the rule will encode (e.g. *"caller equals the owner field"*).
3. The role / population that needs broader visibility (e.g. *"recruiters and HR want to see every note regardless of author"*).
4. The four documented options for the broadening mechanism. The user picks one before Stage 12 completes:

   | Option | What it means | When to pick |
   |---|---|---|
   | **A. Accept the uniform filter** | Drop the broader-audience intent; everyone with `view_permission` sees only what the predicate allows. | The broader audience was nice-to-have, not a requirement. The simplest answer; pick first unless an argument exists. |
   | **B. Encode in the column, not the permission** | Add a column whose value broadens the predicate (e.g. `visibility` enum with a `'team'` value the rule treats as visible to everyone). | The broadening is per-row decided by the author, not per-caller decided by their role. |
   | **C. Split into separate entities / views** | The broader-audience read uses a different table or cube view that admins access via its own `view_permission`. | The broader audience reads aggregates / summaries; access shape genuinely differs. |
   | **D. Platform-level Postgres role bypass** | Specific Postgres roles run with `BYPASSRLS` and skip the policy entirely. The model spec captures the intent; deployment wires the role attribute outside Semantius. | A small set of named operational roles (often: `<slug>:admin`) gets unrestricted read across the module. Requires platform / DBA confirmation that the bypass attribute is set on the role; the analyst NEVER assumes this without explicit user confirmation. |

   Do **not** propose a fifth option ("Just grant a `<slug>:view_all_X` permission and the platform will bypass for holders"). That mechanism is not documented in the current platform spec; promising it in prose produces a rule that silently does not work, which is the failure mode we are explicitly preventing.

5. The recommendation the analyst would pick if forced (typically A or B; D only when the user has named a real DBA-side mechanism).

When the user picks one, encode that decision in §7 (resolved into §7.2 *Future considerations* if it's deferred design, or as an §8 implementation note if the choice has implementation consequences for the deployer). The `select_rule` body itself is the per-row predicate, full stop.

#### Present the scan-table to the user

Show the full scan table per entity AND a compact list of the entities that fired with their proposed JsonLogic. **For every fired cell, surface the read-visibility change explicitly** ("after deploy, every caller with `view_permission` — including recruiters and managers — sees only `<entity>` rows where `<predicate>`. If that is not the intent, this is an architectural decision; pick option A/B/C/D before proceeding.") Read-visibility changes are medium-risk; the user must understand which rows become invisible to whom before signing off, AND must explicitly resolve the broader-audience question for every fired family. Loop on feedback until confirmed.

Each fired entity gets a `Select rule` sub-block in its §3 sub-section with the JsonLogic body. The sub-block's `description` field describes what the per-row predicate *does*, not what a broader audience *should* see — that description must be auditable against the JsonLogic body byte-for-byte. **Never describe a bypass the JsonLogic doesn't encode.** Entities with no fire omit the heading entirely.
...
After Stages 8 (computed/validation), 9 (tier classification), 10 (workflow permissions), 11 (input_type_rule), and 12 (select_rule) have all proposed their rules, run **one final cross-cutting consistency pass** before Stage 13 writes the file. This gate exists because the model's view-and-edit-access story is now spread across five surfaces — `view_permission` + `edit_permission` (set per entity), `validation_rules` with `require_permission` (per entity), `select_rule` (per entity), `input_type_rule` (per field), and §2 Permissions summary (catalog of every permission) — and a defect on any one surface that doesn't reconcile with the others is the kind of bug that ships dangerous-looking access controls. The canonical failure mode it prevents: prose claims a permission bypasses `select_rule`, JsonLogic encodes nothing of the kind, the rule ships looking authoritative, and production access is wrong. The platform applies each rule mechanically and independently; if the rules collectively don't mean what the prose says they mean, no one notices until production.
...
3. **`require_permission(<code>)` × §2 cross-check.** Walk every `validation_rules[].jsonlogic` body across every entity for `{"require_permission": "<code>"}` literals. Every `<code>` must appear in §2. Mismatch is 🔴 Blocker (this check already exists in Stage 8's structural audit; replay it here so the consistency view in one place names all surfaces simultaneously).
...
4. **`select_rule` prose × JsonLogic byte-for-byte audit.** For every entity carrying a non-empty `Select rule` sub-block: the sub-block's `description` text MUST describe what the JsonLogic body actually does, with NO claims the body does not encode. Mechanically:
   ...
   - Tokenize the description for permission-shaped tokens (any `<slug>:<suffix>` or capitalized references to declared permissions). For every such token, confirm the JsonLogic body references it directly (a literal `{"require_permission": "<code>"}` or similar permission-check operator IF the platform supports it in SELECT context). If the description names a permission the body does not reference, the prose claims a bypass the rule does not implement — 🔴 **Blocker** (this is the failure mode we are explicitly preventing).
   - Tokenize the description for bypass-shaped phrases: *"bypass"*, *"elevated"*, *"override"*, *"see every"*, *"unrestricted"*, *"admin-only escape"*, *"RLS bypass"*, *"holders of X see"*. Every match must point at either (a) a JsonLogic clause that encodes it, OR (b) an explicit §7-resolved architectural decision (option D Postgres role bypass, option C separate entity, etc., per Stage 12's "What `select_rule` cannot do" section). A bypass phrase with no matching JsonLogic clause and no §7 decision is a 🔴 Blocker.
...
7. **`select_rule` × `view_permission` semantic check.** When an entity carries BOTH a `select_rule` AND a `view_permission` that the model declares broadly (e.g. `<slug>:read` granted to every operational role), the **uniform-filter consequence applies to everyone with the broad permission**. Re-read the entity's §3 prose and the user's stated intent from Stage 12: does the model genuinely want the same per-row filter applied to every reader? If the design wants tiered visibility (manager sees more than recruiter), the Stage-12 §7.1 architectural decision must already be resolved (option A/B/C/D); if §7.1 still carries an unresolved blocker on this entity, **the gate does not pass** and Stage 13 does not write. Force the user to resolve before save.
...
The `Match?` column for `select_rule` is the byte-for-byte audit: does the prose describe what the JsonLogic does, and nothing more? When ✅, the prose is auditable. When ❌, name the gap and route back to Stage 12's architectural-decision flow.
...
**This gate's whole purpose is to catch the rebuild-style defect where prose promises a bypass the rule does not encode.** Treat consistency between prose and JsonLogic as a load-bearing invariant. Never write the file when this gate's table contains a 🔴 cell.
...
- 🔴 **`select_rule` prose × JsonLogic body match (v2.2 critical rule).** Every claim the sub-block's `description` (or any related §3 / §7 prose about the entity's visibility) makes about *what the rule does* must be encoded in the JsonLogic body. Mechanically, the audit walks the description for:
  - **Permission tokens** (`<slug>:<suffix>` or capitalized references to declared permissions). Every named permission must appear in the JsonLogic body via a literal `{"require_permission": "<code>"}` (or equivalent platform operator IF documented for SELECT context). A permission named in prose but absent from the JsonLogic body is a 🔴 Blocker — the prose claims a bypass the rule does not implement, and the model ships dangerous-looking but broken RBAC. This is the **canonical v2.2 defect** the audit exists to catch.
  - **Bypass-shaped phrases** (`"bypass"`, `"elevated"`, `"override"`, `"see every"`, `"unrestricted"`, `"admin-only escape"`, `"RLS bypass"`, `"holders of X see"`, `"degrade to"`, `"falls back to"`). Every match must point at either (a) a JsonLogic clause that encodes it, OR (b) an explicit §7-resolved architectural decision (option D Postgres role bypass, option C separate entity, etc., per Stage 12's "What `select_rule` cannot do" section). A bypass phrase with no matching JsonLogic clause AND no §7 architectural-decision entry is a 🔴 Blocker. The platform does not honor a bypass it cannot evaluate.
...
- 🔴 **Tiered-visibility intent without architectural decision (v2.2 critical rule).** When the model's §3 prose for an entity carrying a `select_rule` describes a tiered audience ("regular users see X, managers see Y", "external participants see only their own, recruiters see all", or any phrasing implying different roles read different sets), AND no §7-resolved architectural decision (Stage 12 option A/B/C/D) explains how the broader-audience read is implemented, this is a 🔴 Blocker. The `select_rule` JsonLogic applies uniformly to every caller with `view_permission`; tiered visibility cannot be encoded inside the rule alone. Either resolve the architectural decision and capture the resolution in §7, or rewrite the prose to describe the actual uniform filter the rule encodes.
...
- 🟡 **Tiered-audience-shaped entity without a `select_rule`.** Every entity that carries BOTH a submitter-shaped FK AND an assignee-shaped FK — the canonical ticket/case-management shape — carries a `Select rule` block with a submitter-or-assignee-or-unassigned predicate, OR a §7.2 entry documents why every operational user sees every row. Neither present is a 🟡 Warning. Note: the rule applies uniformly to every caller with `view_permission`; if the design wants tiered visibility, that's an architectural decision (see Stage 12 options A/B/C/D), NOT something to encode inside the rule.
...
- 🔴 Every `require_permission(<code>)` argument across all `validation_rules` is a declared §2 row.
- 🔴 Every column referenced inside a `select_rule` body resolves to a real field on that entity.
- 🔴 Every `select_rule` prose claim about permission bypass is encoded in the JsonLogic body OR resolved as a §7 architectural decision (per the v2.2 critical rule above).
...
- **Run Stage 12 (row-level read-access scan) against every newly-added entity AND every existing entity the extension reshaped.** Walk the five S-families (S1 personal-content, S2 tiered-audience, S3 confidential, S4 sensitive HR/financial, S5 time-bounded). Each fire either lands a `Select rule` block on the entity or earns a §7.2 escape (or, when a broader-audience intent surfaces, a §7.1 architectural decision per Stage 12's "What `select_rule` cannot do" section). Read-visibility changes are medium-risk; surface every fire explicitly in the change summary ("after deploy, **every caller with `view_permission` — including recruiters and managers** — sees only `<entity>` rows where `<predicate>`") and confirm with the user before saving. An extension that adds an `assignee_user_id` to a previously-flat entity makes it tiered-audience — propose the matching `select_rule` and warn the user about the uniform-filter visibility change. **Never describe a permission-bypass the JsonLogic does not encode.**
...
12. **Stage 12** — re-run the **mandatory mechanical row-level read-access scan** against every rebuilt entity. Its own confirmation gate, never bundled with Stage 11. Produce the full per-entity scan table (one row per entity, five S-family columns), the compact proposed-rules list, and surface every fired cell to the user **with explicit visibility-change framing** ("after deploy, **every caller with `view_permission`** sees only `<entity>` rows where `<predicate>`"). Read-visibility changes are medium-risk; the user must confirm row-by-row impact before the rebuild saves. Prior `Select rule` blocks are an input but not a substitute for the walk; an entity that gained a `submitter_user_id` / `assignee_user_id` pair in the rebuild becomes tiered-audience-shaped and warrants a `select_rule` even if the prior file had none. **For every fired family where the design wants tiered visibility (broader audience for some roles), surface the Stage 12 "What `select_rule` cannot do" architectural decision (option A/B/C/D) and require user resolution before the gate passes.** Carry-over of prior prose that promises a permission bypass not encoded in JsonLogic is a defect the rebuild MUST correct.
