...
```json
{
  ...
  "description": "Moving an offer into `approved` is the budget commitment step. The static edit_permission on the entity lets recruiters draft and send offers; this rule layers the approval gate on top so only designated approvers can flip the workflow state field itself.",
  "jsonlogic": {
    "if": [
      {
        "and": [
...
The body reads: *"if the workflow state field changed and its new value is `approved`, require the caller to hold `ats:approve_offer`; otherwise the rule passes trivially."* The wrap pattern is **`{"if": [<trigger>, {"require_permission": "..."}, true]}`**, the `true` fallback is mandatory; dropping it makes the rule fail whenever the trigger is false.

Three guidelines for `require_permission`:

1. **The permission code must be declared in the model's permission catalog (§8.1 in blueprint and spec alike).** The deployer cross-checks every `require_permission` argument against that catalog; an undeclared code is a deploy-time blocker, not a runtime surprise. Workflow-specific permissions (`<slug>:approve_<noun>`, `<slug>:close_<noun>`, `<slug>:sign_<noun>`) belong alongside the baseline `<slug>:read` / `<slug>:manage` / `<slug>:admin` triple.
...
Pair this with a `validation_rules` entry on the server side that requires `approved_at` to be non-null whenever `workflow_state` is `approved`; together they give the analyst both the "field appears at the right time" UX and the "the field is actually populated" data invariant.
