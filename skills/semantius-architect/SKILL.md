...
**Major bump rationale for 4.0.** Architect v3.x produced a full spec (entities + fields + JsonLogic + everything). Architect v4.0 produces a blueprint only — field-level content moved to the new `semantius-analyst` skill. The contracts are not compatible; the analyst expects `blueprint_version: "2.0"` on its blueprint input.

**Minor bump rationale for 4.1.** The blueprint format gained new contract carriers without renumbering or removing sections:

1. Frontmatter: `tagline`, `description`, `persona` (derived list), `license`, `module_kind` (informational label). `related_modules` reframed as advisory only — modules deploy standalone; the embedded_master mechanism is the self-sufficiency lever. The "deployable closure / required modules" framing is gone.
2. §3: new `write tier` column (the per-entity edit tier — was previously emitted only as the spec's `**Edit permission:**` annotation; now also a §3 column so the analyst consumes rather than re-derives).
3. §5.1 / §5.2 / §5.3a: new `delete_mode` and `fk_format` columns (resolved at architect time; analyst consumes verbatim). §5.3b introduces the cross-scope `delete_mode` vocabulary `none`, `none (required-if-present)`, `⚠ audit: <reason>`. `is_required` is now presence-conditional everywhere (a required edge becomes a mandatory FK only when the target entity is installed in the same deploy, never forces the target to install).
4. §6.2 / §6.3: new `transition` column carrying `<to_state> _(<event_category>)_`.
5. §7: soft `⚠ state-machine shape` / `⚠ unresolved gate: <reason>` annotations. The architect surfaces; the analyst skips re-modeling; the deployer fail-louds.
6. §8.2: `has_single_approver` rules MUST name the actual approve gate via `permission_verb_override` (e.g. `approve_offer`); phantom `approve_<entity>_approval` codes are an authoring error.
7. New §9: baseline roles + permission hierarchy + RACI realization (polymorphic actor: persona or skill) + functional ownership.
8. Embedded-entity governance completeness: an installing unit carrying an entity as `embedded_master` whose canonical owner is absent at deploy time emits the entity's FULL derived governance under the installing unit's slug (workflow gates + pattern-flag overrides + matching §8.2 rules + boundary-crossing handoffs in §6.2 / §6.3). Governance follows the entity, not the role.

All eight additions are backward-compatible (older files parse on the new analyst with defaulted-empty new fields). The bump is MINOR because no section renumbered or got renamed, and no required key got removed.

**Minor bump rationale for 4.2.** §9.1 RACI gained two additive columns — `process_key` (stable snake_case process id) and `consult_mode` (`read` / `notify` / `block`, consulted rows only) — plus an optional `raci_mode` frontmatter hint. These carry the design-time RACI surface the downstream skills need to drive the platform's now-shipped live-RACI engine (catalog tables `processes` / `raci_assignments` / `process_gates`; operators `is_raci_actor` / `has_consultation`) instead of only compiling RACI into RBAC grants. The architect stays mode-agnostic (names processes, actors, letters, per-consulted mode; no enforcement detail). `blueprint_version` bumps `2.0 → 2.1`. MINOR: additive columns + one optional frontmatter key, no section renumbered, no required key removed; v4.1 (`blueprint_version 2.0`) files still parse on the new analyst — a missing `process_key` is derived from the process name, a missing `consult_mode` defaults to `read`.

**Minor bump rationale for 4.3.** The empty-section convention changed: every canonical top-level / numbered section (plus the §5.3 / §6 sub-blocks) is now **always present**; an intentionally-empty section carries the canonical placeholder `_(none: <short reason>)_` instead of being omitted. This inverts the prior "omit-empties, stubs forbidden" rule (the pre-save verifier now rejects a *missing* canonical section and *accepts* the placeholder). `blueprint_version` bumps `2.1 → 2.2`. MINOR: no section renumbered, no required key removed, no column shape changed. Older blueprints (which omit empty sections) are **grandfathered** — they still parse on the new analyst (which gained tolerance for both "absent = empty" and "present-with-placeholder = empty"); a new-form blueprint's placeholder is inert to older readers (it sits in a section slice and is skipped as a non-row). The bonus: because no canonical section is ever absent, section numbers never shift, neutralizing the parse-by-number brittleness without a parser rewrite. (The §3 per-entity sub-blocks — Computed fields / Validation rules / Input-type rules / Select rule — stay omit-when-empty; they are not numbered navigation anchors.)

**Major bump rationale for 5.0 (provenance-in-platform).** The §3 entities catalog gained two new columns, which is a breaking column-shape change to a structural table (per "When to bump" below), so `blueprint_version` bumps MAJOR `2.2 → 3.0` and this skill bumps MAJOR `4.3 → 5.0` in lockstep:

1. **`canonical code` (D6)** — each entity records its canonical uber-model code beside its local/dialect `data_object`. The deployer stamps it into `entities.catalog_entity_code` (write-once identity), so rename / dialect / silo detection downstream becomes a join, not a heuristic. Agent-optimized greenfield blueprints set `canonical code = data_object`, so the column is free for them; catalog-clones carry the source slice's canonical code.
2. **`entity_type` (D9)** — the closed 6-way data-class axis (`operational_workflow | operational_record | catalog | junction | computed | unclassified`), mirroring upstream `data_objects.entity_type`. Stage 9 is reframed around it: the class is the **input**, and `write tier` is **derived from it** (`catalog → :admin`, workflow/record `→ :manage`, `junction →` neighbor-based, `computed → :read`), never the reverse. The old operational-vs-admin heuristic is preserved and promoted into ladder step 4 (the `catalog` test).

Because columns were inserted mid-table, every downstream §3 parser (the analyst, the shared `consistency-check.ts`) reads §3 **by header name, not column position**. Older `2.x` / `4.x` blueprints (no `canonical code` / `entity_type` columns) route to Mode D Rebuild like any older-major file; the Rebuild re-authors at `3.0` with both columns derived.

**Minor bump rationale for 5.1 (composed record labels).** The platform added read-time composed labels — every entity exposes a read-only `_label` (folded from its parent chain) and every reference/parent FK an `<fk>_label` companion — driven by an optional per-entity `label_parent` (the one FK that is the record's identity spine). This is a **prose-only** clarification for the architect: **no blueprint column is added**, `blueprint_version` stays `3.0`. Two clarifications land: (1) `entity_type = junction` is restated to combine **all** parent legs (binary or N-ary, `(user, role, tenant)` works the same), with the association-class caveat — an N-ary link carrying its own attributes or a lifecycle is `operational_record` / `operational_workflow`, not `junction`; (2) §5 guidance notes that the author MAY flag a multi-parent relational entity's identity spine in the §5 `notes` column for the analyst to turn into `label_parent`. MINOR per the rules below: a tightened modeling convention authors follow, with no section renumbered, no column added, no key removed. Lands alongside analyst `5.2` and the modeler's parse/stamp/verify pass.

**Minor bump rationale for 5.2 (fixed lifecycle state field name + `catalog_field_code` retired).** Two coordinated content-rule changes, both non-breaking. (1) **`workflow_state` is the fixed lifecycle state field name.** Every §7 lifecycle state machine is materialized downstream as a single required `enum` field named exactly `workflow_state` (values = the §7 `state_name`s, default = the `initial?` state); the architect never names the state field `status` / `state` / `lifecycle_state` in any prose surface. This replaces the prior "the state column has no fixed name" tolerance that forced downstream tooling to guess among `status` / `state` / `lifecycle_state`; the deployer now FAILS LOUD on any module that stores lifecycle state under another field name. No blueprint column is added (the field name lives downstream), so `blueprint_version` stays `3.0` — a new modeling convention authors follow, hence MINOR. (2) **`catalog_field_code` is retired** from the shared data-modeling reference (the platform is dropping the `fields.catalog_field_code` column); field-rename detection reverts to the name/format heuristics it always carried as a fallback, and entity / module rename detection via `catalog_entity_code` / `catalog_module_code` is unchanged. Lands in lockstep with analyst `5.3` and the modeler's parse/verify enforcement pass.
...
**Capture the catalog-surface text (architect v4.1+).** Before moving on to Stage 2, elicit the three additional frontmatter strings that drive marketing / catalog surfaces. Each has a distinct audience and surface, and `system_description` (the short display name) is no longer expected to cover all of them:
...
`embedded_master` is the right choice when the blueprint must be deployable today even though the canonical owner doesn't exist yet, but you want the analyst to migrate the entity automatically once the owner installs. The blueprint contract is: *"this entity will belong to `<mastered_in>` once that module is in place; until then, host it here."*

**Presence-conditional `is_required` on §5.3 edges (architect v4.1+).** A `required` cross-scope edge is **presence-conditional**: it becomes a mandatory FK at deploy time only when the target entity is installed in the same deploy. It NEVER forces the target to install. The vocabulary in §5.3b's `delete_mode` column makes this explicit:
...
**`related_modules` describes the system's *neighborhood* in the enterprise, NOT just what's shadowed by §3 entities (v1.7 rule).** The walk has two axes that must both run; collapsing them to just the entity-driven axis is the bug v1.6 shipped.
...
**Event-handoff rows (§6.2 outbound / §6.3 inbound; architect v4.1+):**
...
### Stage 8: Business-rule emission (architect v4.1+)

Computed fields, validation rules, and the 15-family scan walk moved to the analyst skill in architect v4.0. The blueprint stops at entity level: §8 declares permissions and business rules at the intent level only (`rule_name + data_object + source_flag + intent`). The analyst converts each blueprint §8.2 rule into JsonLogic and runs the full scan walk in its Stage 10.

**v4.1 addition — `has_single_approver` names the actual approve gate.** When a §3 entity carries the `single_approver` pattern flag, the §8.2 business rule for that entity MUST name the actual approve gate via `permission_verb_override` in its `intent` text. The named verb (e.g. `approve_offer`) MUST appear in §8.1 as a `workflow-gate (lifecycle)` row whose code is `<system_slug>:<verb>`. Phantom codes like `approve_<entity>_approval` are an authoring error caught by the pre-save verification.

Rationale: under v4.0 the §8.2 rule prose paraphrased the approve gate ("requires an approver"), and the analyst / deployer had to guess the matching permission code. Several models landed with the phantom `approve_<entity>_approval` pattern (a permission name nobody else used or minted), producing dead role-permission rows on deploy. v4.1 closes the loop: the §8.2 rule names the gate directly, and the architect's pre-save verification reads §8.1 to confirm the named code exists.
...
### Stage 11 — Persona discovery and §9 emission (architect v4.1+)
...
- `tagline` (one-line marketing-voice line; v4.1+)
- `description` (longer marketing-voice prose for the catalog page; v4.1+; YAML literal block fine)
- `domain_modules` (typically `[<system_slug>]`)
- `domain_code` (uppercase TLA, e.g. `ATS`, `HCM`, `CRM`)
- `persona` (auto-populated from §9.1 RACI actors; v4.1+)
- `module_kind` (informational label: `domain` / `master` / `starter` / etc.; v4.1+)
...
- **Catalog-Clone only**: `related_modules` (inherited from source; v4.1 reframed as advisory hint, never a prerequisite), `departments` and `industries` (when populated in source). **Do not emit `naming_mode`** — catalog blueprints don't carry it.
...
- Required keys present: `artifact`, `version`, `blueprint_version`, `system_name`, `system_description`, `system_slug`, `tagline` (v4.1+), `description` (v4.1+), `license` (v4.1+), `naming_mode` (greenfield only), `module_kind` (v4.1+), `persona` (v4.1+), `created_at`, `entities`, `initial_request`
...
- `../semantius-analyst/references/data-modeling.md` — Semantius platform reference (entity naming rules, built-in tables, field format rules, relationship rules). Lives with the analyst now because field-level work is the analyst's job; load it if you need to reason about platform constraints during blueprint design.
