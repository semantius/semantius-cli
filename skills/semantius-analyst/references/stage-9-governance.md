...
- Look up `permission_hierarchy` by composite key (`including_permission_id`, `included_permission_id`). Missing → the row's `reconciliation` cell carries `✨ to create` (narrated as `✨ hierarchy edge to be added`; the deployer's Stage 2a-scaffold step 3 mints it); present → `♻ exists`. Mismatched `origin` is 🟡 drift.
...
**Step 5 — Boundary-crossing handoffs and re-prefix annotations.** Per Writing Convention 10 on the architect, the blueprint's §6.2 / §6.3 handoffs for embedded_master entities whose catalog owner is absent are emitted under the installing unit. The analyst carries them verbatim into the spec's §6 `### Outbound handoffs` / `### Inbound handoffs` sub-sections (nine blueprint columns, under both access scopes), with the source module set to the entity's current owning module (which may BE the installing unit, per the entity-owning-module rule). On master-install, the deployer's Stage 4n re-attributes the handoff to the new catalog owner module.
