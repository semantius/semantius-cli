...
- **Paths:** the config dir is `%APPDATA%\semantius` on Windows but `~/.config/semantius` on Linux/macOS. Credentials are not in it: stored sessions live under `getUserSecretsDir()`, which on Windows is `%LOCALAPPDATA%\semantius` — Roaming is the half of the profile built to travel, and a credential belongs on the side that stays on the machine. On Linux/macOS the two are the same directory. Never hardcode either layout; go through `getUserConfigDir()` or `getUserSecretsDir()`.
