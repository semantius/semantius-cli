/**
 * Create Webhook Receiver Tool
 * Creates a new webhook receiver record in the webhook_receivers table
 */
...
const inputSchema = {
  data: oneOrMany(webhook_receiverSchema).describe(
    `Data object containing fields for the new webhook receiver, or a non-empty array of such objects to create several webhook receiver records in one request. ` +
...
export const createWebhookReceiverTool: Tool<typeof inputSchema, undefined> = {
  name: 'create_webhook_receiver',
  options: {
    title: 'Create Webhook Receiver',
    description: `Creates a new webhook receiver record in the webhook_receivers table. ` +
  ...
  handler: async ({ data, accept }, { authInfo }) => {
    try {
      const result = await makePostgrestRequest({
        ...bulkInsertOptions('/webhook_receivers', data, accept),
        method: 'POST',
        body: data,
