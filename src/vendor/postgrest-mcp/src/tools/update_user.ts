...
const inputSchema = {
  id: oneOrMany(z.number().int()).describe('ID of the user to update, or a non-empty array of IDs. When an array is given, the same "data" is applied to every matching record.'),
  data: userSchema.partial().describe(
...
export const updateUserTool: Tool<typeof inputSchema, undefined> = {
  name: 'update_user',
  options: {
    title: 'Update User',
    description: `Updates an existing user record by ID. ` +
      `Provide the user ID and the fields to update. ` +
