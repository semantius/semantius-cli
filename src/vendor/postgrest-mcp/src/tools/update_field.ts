/**
 * Update Field Tool
 * Updates an existing field record by ID
 */
...
const inputSchema = {
  id: oneOrMany(z.string()).describe('ID of the field to update, or a non-empty array of IDs. When an array is given, the same "data" is applied to every matching record.'),
  data: fieldSchema.partial().describe(
...
export const updateFieldTool: Tool<typeof inputSchema, undefined> = {
  name: 'update_field',
  options: {
    title: 'Update Field',
    description: `Updates an existing field record by ID. ` +
  ...
  handler: async ({ id, data, accept }, { authInfo, request }) => {
    try {
