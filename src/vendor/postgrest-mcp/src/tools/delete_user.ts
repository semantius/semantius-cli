/**
 * Delete User Tool
 * Permanently deletes a user record by ID
 */
...
const inputSchema = {
  id: oneOrMany(z.number().int()).describe(
    `ID of the user to delete, or a non-empty array of IDs to delete several records in one request. ` +
...
export const deleteUserTool: Tool<typeof inputSchema, undefined> = {
  name: 'delete_user',
  options: {
    title: 'Delete User',
    description: `Permanently deletes a user record by ID. ` +
