...
import { moduleSchema } from './schemas/moduleSchema.ts'
import { oneOrMany, keyFilter } from '../utils/bulk.ts'

const inputSchema = {
  id: oneOrMany(z.number().int()).describe('ID of the module to update, or a non-empty array of IDs. When an array is given, the same "data" is applied to every matching record.'),
  data: moduleSchema.partial().describe(
...
export const updateModuleTool: Tool<typeof inputSchema, undefined> = {
  name: 'update_module',
  options: {
    title: 'Update Module',
    description: `Updates an existing module record by ID. ` +
