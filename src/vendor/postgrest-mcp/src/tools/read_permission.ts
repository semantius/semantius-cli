/**
 * Read Permission Tool
 * Reads and queries permissions from the permissions table
 */
...
const inputSchema = {
  filters: z
    ...
    .describe(
      ...
      `Examples: "permission_name=eq.crm:read" (get by key, which is the permission name), ` +
      `"module_id=eq.3" (permissions of one module), "permission_name=ilike.crm:*" (prefix search), ` +
      `"permission_name=in.(crm:read,crm:manage)" (multiple values). ` +
...
export const readPermissionTool: Tool<typeof inputSchema, undefined> = {
  name: 'read_permission',
  options: {
    title: 'Read Permission',
    description: `Reads and queries permissions from the permissions table. ` +
