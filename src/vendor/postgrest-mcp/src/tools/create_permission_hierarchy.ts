/**
 * Create Permission Hierarchy Tool
 * Creates a new permission hierarchy record in the permission_hierarchy table
 */
...
const inputSchema = {
  data: oneOrMany(permission_hierarchySchema).describe(
    `Data object containing fields for the new permission hierarchy, or a non-empty array of such objects to create several permission hierarchy records in one request. ` +
...
export const createPermissionHierarchyTool: Tool<typeof inputSchema, undefined> = {
  name: 'create_permission_hierarchy',
  options: {
    title: 'Create Permission Hierarchy',
    description: `Creates a new permission hierarchy record in the permission_hierarchy table. ` +
  ...
  handler: async ({ data, accept }, { authInfo }) => {
    try {
      const result = await makePostgrestRequest({
        ...bulkInsertOptions('/permission_hierarchy', data, accept),
        method: 'POST',
        body: data,
