/**
 * Create Role Permission Tool
 * Creates a new role permission record in the role_permissions table
 */
...
import { role_permissionSchema } from './schemas/role_permissionSchema.ts'
import { oneOrMany, bulkInsertOptions } from '../utils/bulk.ts'

const inputSchema = {
  data: oneOrMany(role_permissionSchema).describe(
    `Data object containing fields for the new role permission, or a non-empty array of such objects to create several role permission records in one request. ` +
...
export const createRolePermissionTool: Tool<typeof inputSchema, undefined> = {
  name: 'create_role_permission',
  options: {
    title: 'Create Role Permission',
    description: `Creates a new role permission record in the role_permissions table. ` +
  ...
  handler: async ({ data, accept }, { authInfo }) => {
    try {
      const result = await makePostgrestRequest({
        ...bulkInsertOptions('/role_permissions', data, accept),
        method: 'POST',
        body: data,
