/**
 * Create User Role Tool
 * Creates a new user role record in the user_roles table
 */
...
const inputSchema = {
  data: oneOrMany(user_roleSchema).describe(
    `Data object containing fields for the new user role, or a non-empty array of such objects to create several user role records in one request. ` +
...
export const createUserRoleTool: Tool<typeof inputSchema, undefined> = {
  name: 'create_user_role',
  options: {
    title: 'Create User Role',
    description: `Creates a new user role record in the user_roles table. ` +
  ...
  handler: async ({ data, accept }, { authInfo }) => {
    try {
      const result = await makePostgrestRequest({
        ...bulkInsertOptions('/user_roles', data, accept),
        method: 'POST',
        body: data,
