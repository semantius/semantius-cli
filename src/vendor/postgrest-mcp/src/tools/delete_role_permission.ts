/**
 * Delete Role Permission Tool
 * Permanently deletes a role permission record by ID
 */
...
const inputSchema = {
  id: oneOrMany(z.string()).describe(
    `ID of the role permission to delete, or a non-empty array of IDs to delete several records in one request. ` +
...
export const deleteRolePermissionTool: Tool<typeof inputSchema, undefined> = {
  name: 'delete_role_permission',
  options: {
    title: 'Delete Role Permission',
    description: `Permanently deletes a role permission record by ID. ` +
