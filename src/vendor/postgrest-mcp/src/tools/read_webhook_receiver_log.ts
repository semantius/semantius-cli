/**
 * Read Webhook Receiver Log Tool
 * Reads and queries webhook receiver logs from the webhook_receiver_logs table
 */
...
export const readWebhookReceiverLogTool: Tool<typeof inputSchema, undefined> = {
  name: 'read_webhook_receiver_log',
  options: {
    title: 'Read Webhook Receiver Log',
    description: `Reads and queries webhook receiver logs from the webhook_receiver_logs table. ` +
  ...
  handler: async ({ filters, select, limit, offset, order, accept }, { authInfo }) => {
    try {
      ...
      let path = '/webhook_receiver_logs'
      if (filters) {
        path += `?${filters}`
        if (params.toString()) {
