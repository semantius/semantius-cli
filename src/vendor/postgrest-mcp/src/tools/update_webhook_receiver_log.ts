/**
 * Update Webhook Receiver Log Tool
 * Updates an existing webhook receiver log record by ID
 */
...
const inputSchema = {
  id: oneOrMany(z.number().int()).describe('ID of the webhook receiver log to update, or a non-empty array of IDs. When an array is given, the same "data" is applied to every matching record.'),
  data: webhook_receiver_logSchema.partial().describe(
...
export const updateWebhookReceiverLogTool: Tool<typeof inputSchema, undefined> = {
  name: 'update_webhook_receiver_log',
  options: {
    title: 'Update Webhook Receiver Log',
    description: `Updates an existing webhook receiver log record by ID. ` +
  ...
  handler: async ({ id, data, accept }, { authInfo }) => {
    try {
      const result = await makePostgrestRequest({
        path: `/webhook_receiver_logs?${keyFilter('id', id)}`,
        method: 'PATCH',
        body: data,
