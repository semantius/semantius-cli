/**
 * Delete Webhook Receiver Log Tool
 * Permanently deletes a webhook receiver log record by ID
 */
...
const inputSchema = {
  id: oneOrMany(z.number().int()).describe(
    `ID of the webhook receiver log to delete, or a non-empty array of IDs to delete several records in one request. ` +
...
export const deleteWebhookReceiverLogTool: Tool<typeof inputSchema, undefined> = {
  name: 'delete_webhook_receiver_log',
  options: {
    title: 'Delete Webhook Receiver Log',
    description: `Permanently deletes a webhook receiver log record by ID. ` +
  ...
  handler: async ({ id, accept }, { authInfo }) => {
    try {
