/**
 * Create Field Tool
 * Creates a new field record in the fields table
 */
...
const inputSchema = {
  data: oneOrMany(fieldSchema).describe(
    `Data object containing fields for the new field, or a non-empty array of such objects to create several field records in one request. ` +
...
export const createFieldTool: Tool<typeof inputSchema, undefined> = {
  name: 'create_field',
  options: {
    title: 'Create Field',
    description: `Creates a new field record in the fields table. ` +
  ...
  handler: async ({ data, accept }, { authInfo, request }) => {
    try {
      const result = await makePostgrestRequest({
        ...bulkInsertOptions('/fields', data, accept),
        method: 'POST',
        body: data,
