/**
 * Read Entity Tool
 * Reads and queries entities from the entities table
 */
...
const inputSchema = {
  filters: z
    ...
    .describe(
      ...
      `Examples: "table_name=eq.1" (get by ID), "email=eq.user@example.com" (exact match), ` +
...
export const readEntityTool: Tool<typeof inputSchema, undefined> = {
  name: 'read_entity',
  options: {
    title: 'Read Entity',
    description: `Reads and queries entities from the entities table. ` +
