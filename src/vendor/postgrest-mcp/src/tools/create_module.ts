...
const inputSchema = {
  data: oneOrMany(moduleSchema).describe(
    `Data object containing fields for the new module, or a non-empty array of such objects to create several module records in one request. ` +
...
export const createModuleTool: Tool<typeof inputSchema, undefined> = {
  name: 'create_module',
  options: {
    title: 'Create Module',
    description: `Creates a new module record in the modules table. ` +
  ...
  handler: async ({ data, accept }, { authInfo }) => {
    try {
      const result = await makePostgrestRequest({
        ...bulkInsertOptions('/modules', data, accept),
        method: 'POST',
        body: data,
