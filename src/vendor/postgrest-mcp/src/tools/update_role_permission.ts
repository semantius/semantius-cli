/**
 * Update Role Permission Tool
 * Updates an existing role permission record by ID
 */
...
const inputSchema = {
  id: oneOrMany(z.string()).describe('ID of the role permission to update, or a non-empty array of IDs. When an array is given, the same "data" is applied to every matching record.'),
  data: role_permissionSchema.partial().describe(
...
export const updateRolePermissionTool: Tool<typeof inputSchema, undefined> = {
  name: 'update_role_permission',
  options: {
    title: 'Update Role Permission',
    description: `Updates an existing role permission record by ID. ` +
  ...
  handler: async ({ id, data, accept }, { authInfo }) => {
    try {
      const result = await makePostgrestRequest({
        path: `/role_permissions?${keyFilter('id', id)}`,
        method: 'PATCH',
        body: data,
