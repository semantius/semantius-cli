/**
 * Update Role Tool
 * Updates an existing role record by ID
 */
...
const inputSchema = {
  id: oneOrMany(z.number().int()).describe('ID of the role to update, or a non-empty array of IDs. When an array is given, the same "data" is applied to every matching record.'),
  data: roleSchema.partial().describe(
...
export const updateRoleTool: Tool<typeof inputSchema, undefined> = {
  name: 'update_role',
  options: {
    title: 'Update Role',
    description: `Updates an existing role record by ID. ` +
