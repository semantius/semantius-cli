/**
 * Delete Webhook Receiver Tool
 * Permanently deletes a webhook receiver record by ID
 */
...
const inputSchema = {
  id: oneOrMany(z.number().int()).describe(
    `ID of the webhook receiver to delete, or a non-empty array of IDs to delete several records in one request. ` +
...
export const deleteWebhookReceiverTool: Tool<typeof inputSchema, undefined> = {
  name: 'delete_webhook_receiver',
  options: {
    title: 'Delete Webhook Receiver',
    description: `Permanently deletes a webhook receiver record by ID. ` +
