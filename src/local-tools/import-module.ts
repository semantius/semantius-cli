...
export default defineLocalTool({
  ...
  description:
    ...
    'Nothing is deleted, and there is no transaction across requests. Validation rules and select_rule are written after the records; an entity that already exists keeps its rules while its records are written.',
  inputSchema: {
    path: z
      .string()
      .min(1)
