...
| Per tenant host, six public discovery documents answer 200 (`tests`, `cli1-bb82`): RFC 9728 `/.well-known/oauth-protected-resource` (and `/mcp`) → `resource …/mcp`, `authorization_servers: [<host>/api/auth]`, `scopes_supported: [tenant:<control-plane id>:user]`; RFC 8414 `/.well-known/oauth-authorization-server` (and `/api/auth`) and OIDC `/.well-known/openid-configuration`, identical keys and values (token `/token`, userinfo `/userinfo`, issuer = `authorization_servers[0]`); `/.well-known/jwks.json` (one Ed25519 key). No document carries `client_id_cli`. | Fetched 2026-09-11 (list from Martin; implementation plan Step 5). |
